...
/**
 * Rows untouched for longer than this are deleted. Keys contain IP addresses and
 * visitor ids (personal data), so they must not be retained indefinitely.
 * Must stay longer than the longest window in use (24 h).
 */
const RETENTION_SEC = 2 * 24 * 60 * 60;
/** Fraction of checks that also prune expired rows (keeps the hot path cheap). */
const PRUNE_PROBABILITY = 0.01;

/**
 * Best-effort deletion of expired rate-limit rows. Never throws.
...
async function pruneExpiredEntries() {
  try {
    await prisma.$executeRaw`
      ...
      WHERE "updatedAt" < NOW() - make_interval(secs => ${RETENTION_SEC}::float)
    `;
  } catch (error) {
    console.warn("[rateLimit] prune failed", { error: error?.message ?? String(error) });
  }
}
...
async function atomicIncrement(key, windowSec) {
  const rows = await prisma.$queryRaw`
    INSERT INTO rate_limit_entries (key, count, "windowStart", "updatedAt")
    VALUES (${key}, 1, NOW(), NOW())
    ON CONFLICT (key) DO UPDATE SET
      count = CASE
        WHEN EXTRACT(EPOCH FROM (NOW() - rate_limit_entries."windowStart")) > ${windowSec}::float
          THEN 1
      ...
      "windowStart" = CASE
        WHEN EXTRACT(EPOCH FROM (NOW() - rate_limit_entries."windowStart")) > ${windowSec}::float
          THEN NOW()
        ELSE rate_limit_entries."windowStart"
      END,
      "updatedAt" = NOW()
...
export async function checkRateLimit(
  ...
  // Replace colons in the identifier so they cannot shift the fixed-position
  // numeric fields (safeMax, safeWindowSec) or collide with scope separators.
  const safeId = normalizedId.replace(/:/g, "|");
  const key = `${normalizedScope}:${safeId}:${safeMax}:${safeWindowSec}`;
