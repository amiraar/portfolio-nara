...
const cleanupTimer = setInterval(() => {
  ...
  for (const [key, entry] of store.entries()) {
    // Use DAY_MS as the longest possible window to keep entries around long enough.
    if (now - entry.windowStart > DAY_MS * 2) {
      store.delete(key);
    }
  }
}, CLEANUP_INTERVAL_MS);
 ...
 * All windows must pass — if any fails, returns the most restrictive result.
 *
 * @param {string} identifier
 * @param {Array<{ maxRequests: number; windowSec: number; scope: string }>} windows
 * @returns {{ allowed: boolean; remaining: number; retryAfter: number; limitedBy: string | null }}
 */
export function checkMultiRateLimit(identifier, windows) {
  let mostRestrictive = { allowed: true, remaining: Infinity, retryAfter: 0, limitedBy: null };

  for (const { maxRequests, windowSec, scope } of windows) {
    const result = checkRateLimit(identifier, maxRequests, windowSec, scope);
    if (!result.allowed) {
      if (!mostRestrictive.limitedBy || result.retryAfter > mostRestrictive.retryAfter) {
        mostRestrictive = { ...result, limitedBy: scope };
      }
    } else if (mostRestrictive.allowed) {
      // Track the smallest remaining across passing windows
