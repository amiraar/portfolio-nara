...
| Screenshots for Racker, QuickCue, CPTool | ✅ Mostly done | 8 of 10 owner-supplied screenshots wired into `public/projects/` (web-servable) and each project's `screenshots` array. 2 held out in `docs/pending-review/` (NOT under `public/`, not web-servable) pending confirmation: `racker-request-detail.png` (real colleague names) and `cptool-candidate-detail.png` (real client name "Riot Games" + real staff name). Move to `public/projects/` and add to the relevant `screenshots` array only after explicit owner go-ahead. |
