...
| **Live site not reflecting any of the above** | 🔲 In progress | Production content is served from the Postgres DB via `/api/portfolio`, not from `lib/portfolioDefaults.js` — that file is only a fallback used when a DB row is missing, and a `projects` row already exists (old descriptions, old bit.ly links, no `caseStudy`/`screenshots` fields). Code changes alone never reach the live site. Fix in progress: added `caseStudy` and `screenshots` fields to `components/dashboard/editors/ProjectsEditor.jsx`, plus a "Load code defaults" button in `ContentEditor.jsx` so the owner can pull the new defaults into the editor and click Save. **Owner action required:** log into `/dashboard` → Content → Projects → "Load code defaults" → review → Save, once this branch is merged and deployed. |
